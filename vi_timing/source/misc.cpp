...
{	assert(decimal < significant);
	...
	else
	{
		...
		{	const auto exp = std::log10(std::abs(num));
			const auto factor = std::pow(10.0, significant - std::ceil(exp));
			num = std::round(num * factor) / factor;

			constexpr auto GROUP = 3;
			const auto supp = ((significant - decimal - 1) / GROUP) * GROUP - exp;
			if (std::islessequal(supp, -6)) { unit = { " Ms", 1e-6 }; }
			else if (std::islessequal(supp, -3)) { unit = { " ks", 1e-3 }; }
			else if (std::islessequal(supp, 0)) { unit = { " s ", 1e+0 }; }
			else if (std::islessequal(supp, 3)) { unit = { " ms", 1e+3 }; }
			else if (std::islessequal(supp, 6)) { unit = { " us", 1e+6 }; }
			else if (std::islessequal(supp, 9)) { unit = { " ns", 1e+9 }; }
		}
