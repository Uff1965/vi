...
namespace
{
	...
	{	static_assert(sizeof(unsigned) >= 4U);
		...
		const auto year = to_num(&date[9]);
		assert(year < 43); // Time coding up to 23:59 on Dec 31 2042 is permitted.
		const auto month = to_month(date + 0);
		const auto day = to_num(&date[4]);
		// "__TIME__ <...> a character string literal of the form 'hh:mm:ss' as in the time generated by the asctime function."
		assert(time[2] == ':' && time[5] == ':' && "The time must be in the __TIME__ macro format.");
		const auto hour = to_num(time + 0);
		const auto minute = to_num(&time[3]);
